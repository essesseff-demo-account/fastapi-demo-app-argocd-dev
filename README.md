# fastapi-demo-app - Argo CD Application (DEV)

This repository contains the Argo CD Application manifest for the **DEV** environment of the fastapi-demo-app essesseff™ app.  
...
```
fastapi-demo-app-argocd-dev/
├── app-of-apps.yaml.template                      # Root Application template
├── argocd/
│   └── fastapi-demo-app-dev-application.yaml.template  # DEV environment Application manifest (auto-synced) template
...
│   └── offboard-fastapi-demo-app-dev.sh   # script for offboarding the fastapi-demo-app dev app 1) from essesseff only or 2) from Argo CD and K8s entirely
├── setup-argocd-cluster.sh           # Argo CD K8s setup script 
├── setup-argocd.sh                   # Argo CD fastapi-demo-app-dev essesseff app setup script 
└── README.md                         # This file
```
...
### Configure fastapi-demo-app-dev essesseff App to Argo CD and deploy to K8s
...
1. **Git Clone This Repository**:
   ```bash
   git clone git@github.com:essesseff-demo-account/fastapi-demo-app-argocd-dev.git
   ```
...
2. **Configure Environment Variables in .env File**:
   ...
   Then set the environment variables in the .env which will be used for generating the following from the templates in this repository:
      ...
      - `fastapi-demo-app-argocd-dev` repository (to read Application manifests)
      - `fastapi-demo-app-config-dev` repository (to read Helm charts and values)
      ...
      c. ***Configuration of fastapi-demo-app-dev Deployment in Argo CD***:

      fastapi-demo-app-dev-application.yaml is used to configure the fastapi-demo-app-dev deployment

      d. ***Configuration of fastapi-demo-app-argocd-dev App-of-Apps Deployment in Argo CD***:

      app-of-apps.yaml is used to configure the fastapi-demo-app-argocd-dev app-of-apps deployment

      e. ***Configuration of fastapi-demo-app-dev Argo CD Notifications to essesseff***:

      If notifications-secret.yaml is downloaded from essesseff for fastapi-demo-app-dev, notifications-configmap.yaml will be used to configure Argo CD notifications to essesseff.
...
4. **Run the setup-argocd.sh Script**:
   ...
   This script applies all secrets, configmaps, Argo CD application definitions, etc. for fastapi-demo-app DEV.
...
5. **Verify in Argo CD UI**:
   ...
   You should see:
   - `fastapi-demo-app-argocd-dev` - Root Application (watches this repository)
   - `fastapi-demo-app-dev` - Environment Application (auto-synced by root Application)

6. **Access the Deployed Application**:
   ```bash
   kubectl port-forward service/fastapi-demo-app-dev 8081:80 -n essesseff-demo-account
   # Access: http://localhost:8081
   ```
### How to Offboard fastapi-demo-app-dev Deployment from Argo CD and K8s

1. **Execute the offboarding script**:
   ```bash
   cd offboarding
   chmod 744 offboard-fastapi-demo-app-dev.sh
   ./offboard-fastapi-demo-app-dev.sh
   ```
...
- **Name**: `fastapi-demo-app-dev`
- **Namespace**: `argocd`
- **Source Repository**: `fastapi-demo-app-config-dev`
...
1. Push code to `main` branch in `fastapi-demo-app` source code repository
2. GitHub Actions builds container image
3. essesseff GitHub App automation triggers essesseff to auto-update Helm `fastapi-demo-app-config-dev/Chart.yaml` and `fastapi-demo-app-config-dev/values.yaml` with the image tag of the newly built image
4. Argo CD syncs DEV Application automatically on K8s

### Manual GitOps Deployment

1. Push update(s) to the `main` branch in `fastapi-demo-app-config-dev` repository, typically to Helm Chart.yaml and/or values.yaml
2. Argo CD syncs DEV Application automatically on K8s

## Repository URLs

- **Source**: `https://github.com/essesseff-demo-account/fastapi-demo-app`
- **Config DEV**: `https://github.com/essesseff-demo-account/fastapi-demo-app-config-dev`
- **Argo CD DEV**: `https://github.com/essesseff-demo-account/fastapi-demo-app-argocd-dev` (this repo)
...
- Allow Argo CD to detect changes in `argocd/fastapi-demo-app-dev-application.yaml` more quickly
